...
export class ProfesorPaypalAccountComponent implements OnInit {
  form: FormGroup;
  isEditMode = false;
  ...
  ngOnInit(): void {
    ...
    if (this.profesorId !== null) {
      this.cursoService.getProfesorPayPalAccountByProfesorId(this.profesorId).subscribe(
        (data) => {
          if (data) {
        ...
        (error) => {
          console.error('Error al obtener la cuenta de PayPal del profesor:', error);
        }
      );
    }
  }
  ...
  onSubmit() {
    ...
    if (this.isEditMode) {
      this.cursoService.updateProfesorPayPalAccount(formData.profesor_id!, formData).subscribe(
        () => {
          Swal.fire('Success', 'Cuenta de PayPal actualizada correctamente', 'success');
          this.router.navigate(['/']);
        },
        (error) => {
          console.error('Error al actualizar la cuenta de PayPal:', error);
          Swal.fire('Error', 'Error al actualizar la cuenta de PayPal', 'error');
        }
      );
    } else {
      this.cursoService.createProfesorPayPalAccount(formData).subscribe(
        () => {
          Swal.fire('Success', 'Cuenta de PayPal creada correctamente', 'success');
